...
function renderContent(content: string) {
  return content.split("\n").map((line, index) => {
    if (line.startsWith("## "))
      return (
        <h2 key={index} id={line.slice(3).toLowerCase().replace(/[^a-z0-9]+/g, "-")} className="mt-10 scroll-mt-28 text-2xl font-bold">
          {line.slice(3)}
        </h2>
      );
    if (line.startsWith("- "))
      return <li key={index} className="text-muted-foreground ml-5 list-disc">{line.slice(2)}</li>;
    if (/^\d+\.\s/.test(line))
      return <li key={index} className="text-muted-foreground ml-5 list-decimal">{line.replace(/^\d+\.\s/, "")}</li>;
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  return (
    <div className="container-site py-12">
      ...
      <JsonLd data={articleJsonLd({ title: post.title, description: post.excerpt, path: `/blog/${post.slug}`, datePublished: post.publishedAt, author: post.author })} />
      ...
      <div className="mb-8 overflow-hidden rounded-3xl border border-white/10">
        <div className="from-primary/40 to-indigo/30 flex min-h-56 items-end bg-gradient-to-br via-[#1a1f2e] p-8">
          <div>
            <p className="text-primary font-mono text-xs tracking-widest uppercase">{post.category}</p>
            <h1 className="mt-2 max-w-3xl text-3xl font-bold sm:text-4xl">{post.title}</h1>
            <p className="text-muted-foreground mt-3 text-sm">{post.author} · {post.publishedAt} · {post.readTimeMinutes} min read</p>
          </div>
        </div>
      </div>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 space-y-3">
            ...
            <ul className="text-muted-foreground space-y-2 text-sm">
              {headings.map((h) => (
                <li key={h}>
                  <a href={`#${h.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} className="hover:text-foreground">{h}</a>
                </li>
              ))}
            </ul>
          </div>
        ...
        <article className="min-w-0">
          ...
          <GlassCard className="mt-12 space-y-3" hover={false}>
            ...
            <div className="flex gap-2">
              <Button className="rounded-xl" type="button">Yes</Button>
              <Button variant="outline" className="rounded-xl" type="button">No</Button>
            </div>
            <p className="text-muted-foreground text-xs">Share: copy the URL from your browser address bar.</p>
          </GlassCard>
