...
const gameDetails: Record<string, { genre: string; developer: string; platform: string; overview: string[]; notice: string }> = {
  isle: {
    ...
    notice: "Availability of products may change with game patches. The status badge on each product page reflects the current state.",
  },
  naraka: {
    ...
    notice: "Naraka receives frequent updates. Always check the product status page before purchasing.",
  },
};
...
export default async function GamePage({ params }: PageProps) {
  ...
  return (
    <div className="pb-20">
      <JsonLd data={breadcrumbJsonLd([{ name: "Home", path: "/" }, { name: game.name, path: `/games/${game.slug}` }])} />

      <section className="border-b border-white/10 py-16" style={{ background: `radial-gradient(circle at top left, ${game.accent}20, transparent 50%)` }}>
        <div className="container-site">
          ...
          <div className="grid gap-10 md:grid-cols-2">
            <div className="max-w-xl">
              <span className="mb-3 inline-block font-mono text-xs tracking-[0.2em] uppercase" style={{ color: game.accent }}>Game overview</span>
              <h1 className="text-4xl font-bold tracking-tight text-white sm:text-5xl">{game.name}</h1>
              <p className="mt-4 leading-relaxed text-white/60">{game.description}</p>
              <div className="mt-6 flex flex-wrap gap-3">
                <Button asChild className="rounded-xl">
                  <Link href={`/cheats/${game.cheatsSlug}`}>View cheats<ArrowRight className="ml-1.5 size-4" /></Link>
                </Button>
              </div>
            </div>
            {details && (
              <div className="rounded-2xl border border-white/10 bg-[#0d1117] p-5">
                <h2 className="mb-4 text-sm font-semibold text-white/70 uppercase tracking-wider">Game info</h2>
                <dl className="space-y-3 text-sm">
                  <div className="flex items-start gap-3">
                    <Gamepad2 className="mt-0.5 size-4 shrink-0 text-white/30" />
                    <div><dt className="text-white/40">Genre</dt><dd className="text-white/80">{details.genre}</dd></div>
                  </div>
                  <div className="flex items-start gap-3">
                    <Shield className="mt-0.5 size-4 shrink-0 text-white/30" />
                    <div><dt className="text-white/40">Developer</dt><dd className="text-white/80">{details.developer}</dd></div>
                  </div>
                  <div className="flex items-start gap-3">
                    <Globe className="mt-0.5 size-4 shrink-0 text-white/30" />
                    <div><dt className="text-white/40">Platform</dt><dd className="text-white/80">{details.platform}</dd></div>
                  </div>
                </dl>
              </div>
            )}
        ...
        <section className="container-site py-12">
          <div className="max-w-3xl space-y-4">
            {details.overview.map((para, i) => (
              <p key={i} className="leading-relaxed text-white/60">{para}</p>
            ))}
      ...
      <section className="container-site py-10">
        <SectionHeading title="Available tiers" description="Three tiers available — Xray for visibility tools, Pro for competitive assists, Private for the full suite." align="left" />
        <div className="grid items-stretch gap-6 md:grid-cols-3">
          {cheats.map((cheat, index) => <CheatCard key={cheat.slug} cheat={cheat} featured={index === 1} />)}
        </div>
      </section>

      <section className="container-site py-12">
        <SectionHeading title="Feature comparison" description="See what each tier includes before purchasing." align="left" />
      ...
      <section className="container-site py-10">
        <div className="rounded-2xl border border-white/10 bg-[#0d1117] p-6 md:p-8 text-center">
          ...
          <p className="mt-2 text-white/50">Visit the {game.name} cheat catalog to compare tiers and check current availability.</p>
          <Button asChild className="mt-5 rounded-xl" size="lg">
            <Link href={`/cheats/${game.cheatsSlug}`}>View {game.shortName} cheats<ArrowRight className="ml-1.5 size-4" /></Link>
          </Button>
        </div>
      </section>
    </div>
