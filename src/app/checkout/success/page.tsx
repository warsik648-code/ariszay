...
export default async function CheckoutSuccessPage({
  ...
  return (
    <div className="container-site flex min-h-[70vh] flex-col items-center justify-center py-16 text-center">
      ...
      {orderId && (
        <p className="mt-2 font-mono text-sm text-white/40">Order #{orderId.slice(-8).toUpperCase()}</p>
      )}
      ...
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        <Button asChild className="rounded-xl">
          <Link href="/account"><Package className="mr-1.5 size-4" />View my orders</Link>
        </Button>
        <Button asChild variant="outline" className="rounded-xl">
          <Link href="/">Back to home<ArrowRight className="ml-1.5 size-4" /></Link>
        </Button>
      </div>
    </div>
  );
