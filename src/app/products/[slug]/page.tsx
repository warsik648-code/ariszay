...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!product) return {};
  return { title: `${product.name} for Gaming`, description: product.description };
}
...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="container-site py-10 pb-28 md:pb-16">
      <JsonLd data={breadcrumbJsonLd([{ name: "Home", path: "/" }, { name: "Products", path: `/products/${product.slug}` }, { name: product.name, path: `/products/${product.slug}` }])} />
      <JsonLd data={productJsonLd({ name: product.name, description: product.description, path: `/products/${product.slug}`, price })} />

      <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr]">
        <div className="space-y-8">
          <DetailHero
            breadcrumb={[{ label: "Home", href: "/" }, { label: "Products", href: "/products/ugc" }, { label: product.name }]}
            title={product.name}
        ...
        <div className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          <HowToGetStarted />
          <WhatsIncluded features={features} featureCount={product.featureCount} />
